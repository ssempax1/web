...
app.config['MYSQL_DATABASE_DB'] = 'tblGradesImportData'
mysql.init_app(app)
...
@app.route('/tblGradesImport/new', methods=['GET'])
def form_insert_get():
    return render_template('new.html', title='New tblGradesImport Form')


@app.route('/tblGradesImport/new', methods=['POST'])
...
@app.route('/api/v1/tblGradesImport', methods=['GET'])
def api_browse() -> str:
    cursor = mysql.get_db().cursor()
    cursor.execute('SELECT * FROM tblGradesImport')
    result = cursor.fetchall()
...
@app.route('/api/v1/tblGradesImport/<int:grade_id>', methods=['GET'])
...
@app.route('/api/v1/tblGradesImport/', methods=['POST'])
def api_add() -> str:
